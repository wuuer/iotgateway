...
/*---------------------------*/
    ...
    [StructLayout(LayoutKind.Explicit)]
    public unsafe struct IODBPSD_3
    {
        [ FieldOffset( 0 )]
        public short   datano ;    /* data number */
        [ FieldOffset( 2 )]
        public short   type ;      /* axis number */
        // Use fixed-size buffer (max size is 32*4=128 for MAX_AXIS=32)
        [ FieldOffset( 4 )]
        public fixed byte _data[128];
        
        // Accessor properties for union fields - these provide views into the same memory
        public byte[] cdatas
        {
            get
            {
                byte[] result = new byte[MAX_AXIS];
                fixed (byte* p = _data)
                {
            ...
            set
            {
                if (value != null)
                {
                    int copyLen = Math.Min(value.Length, MAX_AXIS);
                    fixed (byte* p = _data)
                    {
        ...
        public short[] idatas
        {
            get
            {
                short[] result = new short[MAX_AXIS];
                fixed (byte* p = _data)
                {
                    short* src = (short*)p;
                    for (int i = 0; i < MAX_AXIS; i++)
...
/*-------------*/
    ...
    [StructLayout(LayoutKind.Explicit)]
    public unsafe struct ODBMDL_2
    {
        [ FieldOffset( 0 )]
        public short   datano;
        [ FieldOffset( 2 )]
        public short   type;
        // Use fixed-size buffer (35 bytes)
        [ FieldOffset( 4 )]
        public fixed byte _data[35];
        
        // Accessor properties for union fields
        public byte[] g_1shot
        {
            get
            {
        ...
        public byte[] g_rdata
        {
            ...
            set
            {
                if (value != null)
                {
                    int copyLen = Math.Min(value.Length, 35);
                    fixed (byte* p = _data)
                    {
                        for (int i = 0; i < copyLen; i++)
                        {
